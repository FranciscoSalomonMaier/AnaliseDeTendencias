...
export class MetricsService {
  calculateMany(items: TrendItem[]): AnalyzedTrendItem[] {
    ...
    const preparedItems = items.map((item) => {
      ...
      return {
        ...
        ageInHours,
        views,
        viewsPerHour: this.calculateViewsPerHour(views, ageInHours),
    ...
    const analyzedItems = preparedItems.map((prepared) => {
      ...
      const recencyScore = this.calculateRecencyScore(prepared.ageInHours);
      ...
      return {
        ...
        calculatedMetrics: {
          ...
          engagementRate: this.round(prepared.engagementRate, 2),
    ...
    return analyzedItems
      .sort(
        (first, second) =>
          second.calculatedMetrics.trendScore -
          first.calculatedMetrics.trendScore,
      )
      .map((item, index) => ({
        ...item,
        calculatedMetrics: {
  ...
  private calculateViewsPerHour(views: number, ageInHours: number): number {
    if (views <= 0) {
      return 0;
    }
  ...
  private normalizeScore(value: number, maximumValue: number): number {
